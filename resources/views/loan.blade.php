...
@section('content')
    <div class="container px-4 py-8 mx-auto">
        ...
        @if (session('success'))
            <div class="px-4 py-3 mb-6 text-green-700 bg-green-100 border border-green-400 rounded-md" role="alert">
                {{ session('success') }}
            </div>
        @endif
        @if (session('error'))
             <div class="px-4 py-3 mb-6 text-red-700 bg-red-100 border border-red-400 rounded-md" role="alert">
                {{ session('error') }}
            </div>
        @endif
                ...
                <tbody class="text-sm text-gray-700 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($loans as $loan)
                        <tr class="transition duration-300 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td class="px-6 py-4 dark:text-white">{{ $loan->actual_returned_at ? $loan->actual_returned_at->format('d M Y') : '-' }}</td>
                            ...
                            <td class="px-6 py-4">
                                @if ($loan->status === 'pending_return')
                                    <form action="{{ route('loans.confirmReturn', $loan->id) }}" method="POST" onsubmit="return confirm('Konfirmasi pengembalian buku ini?')">
                                        @csrf
                                        <button type="submit" class="px-3 py-1 text-sm font-medium text-white bg-green-600 rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                            Confirm
                                        </button>
                                    </form>
                                @else
                                    <a href="{{ route('loans.show', $loan->id) }}" class="font-medium text-indigo-600 dark:text-indigo-400 hover:underline">Detail</a>
                                @endif
                            </td>
                        </tr>
                    @empty
