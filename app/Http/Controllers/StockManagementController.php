...
class StockManagementController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Gabungkan data
        $collection = $entries->concat($exits);

        // Search (Collection filter)
        if (!empty($search)) {
            $collection = $collection->filter(function ($item) use ($search) {
                return str_contains(strtolower($item['type']), strtolower($search)) ||
                    str_contains(strtolower($item['book']->title), strtolower($search)) ||
                    str_contains(strtolower($item['category']), strtolower($search)) ||
                    str_contains((string) $item['amount'], $search) ||
                    str_contains((string) $item['stock_before'], $search) ||
                    str_contains((string) $item['stock_after'], $search);
            });
        }
        ...
        // Grouping by book_id
        $grouped = $collection->groupBy('book_id');
